...
### Este projeto foi desenvolvido para o curso da Trybe
...
# Lista de Endpoints
## POST /user
- O endpoint adiciona um novo user à tabela no banco de dados;
...
- Caso algum desses campos seja inválido ou não exista um usuário correspondente no banco de dados, retorna um código de status 400 com o corpo `{ message: "Campos inválidos" }`.

- Caso esteja tudo certo com o login, a resposta deve ser um token `JWT`.

## GET /user

- Retorna todos os users na seguinte estrutura:
...
- Caso a Categoria não contenha o `name` a API retorna um erro de `status 400`.
...
- Caso o post não contenha o `title`, `content` ou `categoryIds` a API retorna um erro de `status 400`.

- A requisição deve ter o token de autenticação nos headers e, caso contrário, retorna um código de `status 401`.

## GET /post

- Esse endpoint lista todos os _BlogPosts_ e os retorna na seguinte estrutura:
